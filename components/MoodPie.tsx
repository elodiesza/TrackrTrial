...
const MoodPie = ({  moods, year, month, daysInMonth, pieWidth }) => {
  ...
  if (moods.length!==0){
    for(var i=1; i<daysInMonth+1;i++){
      moods.filter(c=>(c.year==year&&c.month==month&&c.day==i)).length==0? undefined: moodCounts.filter(c=>c.mood==moods.filter(c=>(c.year==year&&c.month==month&&c.day==i)).map(c=>c.mood)[0])[0].count++;
    }
  }

  const moodColors = ['green','yellowgreen','yellow','orange','red','pink','plum','lightblue'];
  const totalCount = moods.filter(c=>(c.year==year&&c.month==month)).length==0? 0 : moodCounts.map(c=>c.count).reduce((a,b)=>a+b,0);
